...
foreach ($project_ids as $i => $project_id) {
  $url_publications = "https://api.openaire.eu/search/publications?projectID=" . urlencode($project_id) . 
          "&funder=" . $funders[$i] . "&format=json&size=0";
  
  $url_datasets = "https://api.openaire.eu/search/datasets?projectID=" . urlencode($project_id) . 
          "&funder=" . $funders[$i] . "&format=json&size=0";
