...
class Search {
  private function packParamsJSON($params_array, $post_params) {
    ...
    foreach ($params_array as $entry) {
        $current_params = CommUtils::getParameter($post_params, $entry);
...
private function cleanQuery($dirty_query, $transform_query_tolowercase, $add_slashes) {
...
function search($service, $dirty_query
    ...
    $apiclient = new APIClient($ini_array);
    ...
    $query = ($do_clean_query === true)
        ? ($this::cleanQuery($dirty_query, $transform_query_tolowercase, $add_slashes))
        : ($dirty_query);
    ...
    $params_json = $this::packParamsJSON($param_types, $post_params);

    $params_for_id_creation = ($params_for_id === null)?($params_json):($this::packParamsJSON($params_for_id, $post_params));
    ...
    if (!Toolkit::isJSON($output_json) || $output_json == "null" || $output_json == null) {
    ...
    $input_json = json_encode($this::utf8_converter($result));
