...
cooc["titleabstract"] = paste(cooc$title, cooc$abstract, cooc$journal, cooc$author, cooc$subject, sep=" ")
dates = as.Date(cooc$year)
cooc$year = format(dates, format="%B %d %Y")


metadata = cooc
metadata$titleabstract <- NULL

m <- list(content = "titleabstract", id = "id", title = "title", counter = "readers"
