...
get_papers <- function(query, params, limit=NULL) {
  ...
  olog$info(paste("map_id:", .GlobalEnv$MAP_ID, query))
  start.time <- Sys.time()
  # parse params
  ...
  error = function(err){
    olog$warn(paste0("map_id:", .GlobalEnv$MAP_ID, "publications: ", err))
  ...
  error = function(err){
    olog$warn(paste0("map_id:", .GlobalEnv$MAP_ID, "datasets: ", err))
    datasets_metadata <-
    return (data.frame())
  })
  ...
  olog$info(paste("map_id:", .GlobalEnv$MAP_ID, "Time taken:", time.taken, sep=" "))
