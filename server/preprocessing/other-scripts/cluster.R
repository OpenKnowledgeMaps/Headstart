...
create_clusters <- function(distance_matrix, max_clusters=-1, method="ward.D") {
  if (nrow(distance_matrix) < 2) {
    ...
    vclog$info(paste("map_id:", MAP_ID, "Attempts to find k:", attempt))
    ...
    if(!is.null(num_clusters) && max_clusters > -1 && num_clusters > max_clusters) {
      ...
      if(num_items >= 150) {
        vclog$warn(paste("map_id:", MAP_ID, "High content number, increasing max_k."))
    ...
    if(num_items <= 30){
      vclog$warn(paste("map_id:", MAP_ID, "Low content number, lowering max_k."))
    ...
    vclog$info(paste("map_id:", MAP_ID, "Number of Clusters:", num_clusters, sep=" "))
  ...
  if (nrow(distance_matrix) <= 2) {
    points <- tryCatch({
      ord <- metaMDS(distance_matrix, k = 2, parallel = 7, trymax=30,
                     ...
                     halfchange = TRUE)
      vclog$info(paste("map_id:", MAP_ID, "NMDS-Stress:", min(ord$stress), sep=" "))
      points <- ord$points
    }, error=function(err){
