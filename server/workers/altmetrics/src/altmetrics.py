...
class AltmetricsClient(RWrapper):
    ...
    def __init__(self, *args):
        ...
        self.separation = 1.5
        self.rate_key = 'altmetrics-ratelimit'
    ...
    def next_item(self):
        queue, msg = self.redis_store.blpop("altmetrics")
        msg = json.loads(msg.decode('utf-8'))
        k = msg.get('id')
        params = self.add_default_params(msg.get('params'))
        params["service"] = "altmetrics"
        endpoint = msg.get('endpoint')
        return k, params, endpoint

    def altmetrics_rate_limit_reached(self):
        """
    ...
    def run(self):
        while True:
            while self.altmetrics_rate_limit_reached():
