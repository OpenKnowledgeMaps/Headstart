...
def app():
    ...
    with app.app_context():
        ...
        with engine.connect() as connection:
            connection.execute(text("DROP TABLE IF EXISTS visualizations;"))
            connection.execute(text("DROP TABLE IF EXISTS revisions;"))
...
def test_search_api_reachability(app, test_client):
    # Test that the base URL returns a 200 (OK) HTTP status code
    response = test_client.get('http://api/api/stable/persistence/search')
    assert response.status_code == 200
    assert response.content_type == "application/json"
    data = json.loads(response.get_data(as_text=True))
