...
class DBConnection extends Connection {
    ...
    public function writeCoocFile($conference_id, $cut_off, $file_path) {
        ...
        $file_out = Toolkit::openOrCreateFile($file_path);
    ...
    public function writeMetadataFile($conference_id, $file, $cut_off) {
        ...
        while ($row = mysql_fetch_assoc($result_authors)) {
            Toolkit::addOrInitiatlizeArrayKey($paper_authors, $row['contentID'], $row['name']);
        }
    ...
    protected function getBookmarks($conference_id, $cut_off) {
        ...
        while ($row = mysql_fetch_assoc($result)) {
            $bookmark_array[] = $row;
            Toolkit::addOrInitiatlizeArrayKeyNumerical($this->numBookmarks, $row['contentID']);
        }
        
        if(count($this->numBookmarks) > 0) {
            $this->numBookmarks = array_filter($this->numBookmarks, array(new HigherThanFilter($cut_off), 'isHigher'));
        }
    ...
    protected function greaterThanCutoff ($item, $cut_off) {
        return $item >= $cut_off;
    }
