...
papers.drawPapers = function (bubbles) {
    ...
    var nodes = my_canvas.chart.selectAll("g.node")
            .data(bubbles.data)
            .enter().append("g")
            .attr("class", "paper")
...
papers.paper_click = function (d) {
    if (!this.is("loading")) {
        if (!headstart.is_zoomed) {
            var current_node = my_canvas.chart.selectAll("circle")
                    .filter(function (x) {
                        if (d !== null) {
...
papers.applyForce = function (bubbles) {
    ...
    requestAnimationFrame(function render() {
        ...
        current_bubbles.areas_array.forEach(function (a, i) {
            ...
            if (a.x - a.r < 0 ||
                    a.x + a.r > my_canvas.current_vis_size ||
                    a.y - a.r < 0 ||
                    a.y + a.r > my_canvas.current_vis_size) {

                a.x += (my_canvas.current_vis_size / 2 - a.x) * alpha * multiplier_areas;
                a.y += (my_canvas.current_vis_size / 2 - a.y) * alpha * multiplier_areas;
            }
...
papers.drawEntity = function (entity, alpha, count, forced=false) {
    ...
    if (forced || alpha > 0.09 && count % 2 === 0 ||
        ...
        my_canvas.chart.selectAll(entity)
                .attr("transform", function (d) {
                    return "translate(" + d.x + "," + d.y + ")";
                });
    }
...
papers.resizePaper = function (d, holder_div, resize_factor, color) {
    ...
    var max_size = my_canvas.current_vis_size / 2;
