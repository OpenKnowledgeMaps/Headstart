 ...
 */
const getQueryTerms = (context: Context): string[] => {
  ...
  const phraseRegex = /"(.*?)"/g;
  let phraseArray: string[] = cleanedQuery.match(phraseRegex) || [];
  if (phraseArray === null) {
    phraseArray = [];
  }
  ...
  phraseArray = phraseArray.concat(queryWtPhrases.trim().split(/\s+/));
