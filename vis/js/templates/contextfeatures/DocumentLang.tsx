...
const DocumentLang: FC<DocumentLangProps> = ({ value, popoverContainer }) => {
  ...
  // popover messages
  const allLangMessage =
    "All languages were taken into consideration in the creation of this visualisation. Please note " +
    "that not all of them may appear in the visualisation. ";
  const multLangMessage =
    "The following languages were taken into consideration in the creation of this visualisation. " +
  ...
  if (typeof value === "string") {
    const selectedLangOption = LangOptions.find(
      (langOption) => langOption.id === value
    );
    lang = selectedLangOption ? selectedLangOption.label : "";
    popoverText =
      value === "all-lang"
        ? allLangMessage
        : (lang.length > 10 && selectedLangOption?.label) || "";
  ...
  if (Array.isArray(value)) {
    if (value.length === 1 && value[0] === "all-lang") {
      ...
      lang = LangOptions.filter((item) => item.id === value[0] && item)[0]
        .label;
      ...
      lang_list = value
        .map(
          (langId) =>
            LangOptions.find((langOption) => langOption.id === langId)?.label
        )
        .join(", ");
